...
## set <- data.frame(vkn = rep(0,p), mkn = rep(0,p), pkn = rep(0,p), truth = beta_true)
...
for(seedB in 1:nrep){
  ...
  ## PFER-derandomized knockoffs
  ##   res <- pfer_kn(X, Y, v0, M, tau, Sigma, diags, "gaussian")
  ##   rej <- res$rej
  ##   fdp <- sum(beta_true[rej]==0) / max(length(rej), 1)
  ##   power <- sum(beta_true[rej]!=0) / max(k,1)
  ##   pkn_res <- data.frame(method = "pfer", power = power, fdp = fdp, seedB = seedB)
  ##   set$pkn[rej] <- set$pkn[rej] + 1

  ## Save the results
  ##   all_res <- rbind(all_res, vkn_res, mkn_res, pkn_res)
